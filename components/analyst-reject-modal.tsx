...
export function AnalystRejectModal({ applicationId }: { applicationId: string }) {
  ...
  return (
    <>
      ...
      <dialog
        className="w-[min(92vw,32rem)] rounded-2xl border border-tint/10 bg-background p-0 text-ink shadow-[0_30px_80px_rgba(0,0,0,0.55)] backdrop:bg-black/60"
      ...
      >
        <form action={rejectAnalystApplicationAction} className="grid gap-4 p-5">
          ...
          <textarea
            className="min-h-32 w-full rounded-md border border-tint/10 bg-tint/[0.03] px-4 py-3 text-sm text-ink outline-none transition focus:border-accent focus:ring-2 focus:ring-accent/30"
